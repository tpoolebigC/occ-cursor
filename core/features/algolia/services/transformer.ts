...
export async function transformAlgoliaResults(hits: AlgoliaProduct[]) {
  ...
  return hits.map((hit) => {
    ...
    const transformedProduct = {
      ...
      // Add other fields from hit, excluding those already explicitly defined
      objectID: hit.objectID,
      ...Object.fromEntries(
        Object.entries(hit).filter(([key]) => 
          !['entityId', 'name', 'path', 'brand', 'defaultImage', 'prices'].includes(key)
        )
      ),
    };
