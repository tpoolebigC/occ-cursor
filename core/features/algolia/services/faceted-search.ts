  ...
  async (params: AlgoliaSearchParams): Promise<AlgoliaSearchResult> => {
    ...
    if (brand) {
      ...
      if (brandArray.length > 0) {
        ...
        const brandNames = brandArray.map(id => facetMappings.brands[Number(id) as keyof typeof facetMappings.brands]).filter(Boolean);
    ...
    try {
      ...
      const results = await index.search([{
        indexName: INDEX_NAME,
        // query: term || '*', // Ensure we use wildcard for empty terms - removed due to type mismatch
        ...algoliaSearchParams
      }]);
      ...
      const searchResultNbHits = (searchResult as any)?.nbHits || 0;
      const searchResultFacets = (searchResult as any)?.facets;
      console.log('✅ [Algolia] Faceted search found:', searchResultNbHits, 'results');
      console.log('🔍 [Algolia] Raw facets from Algolia:', JSON.stringify(searchResultFacets, null, 2));
      ...
      if (searchResultFacets) {
        console.log('🔍 [Algolia] Available facet keys:', Object.keys(searchResultFacets));
      }
      
      // Transform Algolia results to match BigCommerce format
      const hits = (searchResult as any)?.hits || [];
      const transformedProducts = await searchResultsTransformer(hits);
      
      // Build facets from Algolia's facet data, or from search results if facets are empty
      let builtFacets = [];
      if (searchResultFacets && Object.keys(searchResultFacets).length > 0) {
        console.log('🔍 [Algolia] Building facets from Algolia data:', Object.keys(searchResultFacets));
        builtFacets = buildFacetsFromAlgoliaFacets(searchResultFacets, params);
        console.log('🔍 [Algolia] Built facets:', builtFacets.map((f: any) => ({ name: f.name, type: f.__typename })));
      } else {
        // Fallback: build facets from search results
        console.log('🔍 [Algolia] Building facets from search results (fallback)');
        builtFacets = buildFacetsFromSearchResults(hits, params);
      }

      // Calculate pagination info
      const totalPages = Math.ceil(searchResultNbHits / limit);
